...
const NewAudioContext = createContext<AudioContextType>({
  isPlaying: false,
  isLoaded: false,
  duration: 0,
  position: 0,
...
// Provider component
export const NewAudioProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return <NewAudioContext.Provider value={contextValue}>{children}</NewAudioContext.Provider>;
};

// Custom hook for using the audio context
export const useAudio = () => useContext(NewAudioContext);
