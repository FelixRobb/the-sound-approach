    ...
    signUp: async (email: string, password: string, bookCode: string) => {
      try {
        ...
        if (bookCodeData.activations_used >= bookCodeData.max_activations) {
          dispatch({ type: "AUTH_ERROR", error: "This book code has reached its maximum number of activations" })
          return
        }
        ...
        if (data?.user && data?.session) {
          ...
          const { data: bookCodeData } = await supabase.from("book_codes").select("id").eq("code", bookCode).single()
